...
/**
 * Removes the specified asset from the asset manager.
 * @param {string} id - Identifier of the asset to be removed.
 */
export function deleteAsset(id) {
    if (!id) {
        throw new Error('No asset identifier specified for deletion.');
    }

    const asset = ASSET_MAP.get(id);
    if (asset.packageRefCount) {
        throw new Error('Deleted asset is still has package references.');
    }
